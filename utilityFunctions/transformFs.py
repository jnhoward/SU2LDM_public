...
def transformF(V, FMatrix_old, DEBUG=True):
    ...
    """   
#     FMatrix_new = np.einsum('ae,bf,cg,dh,efgh -> abcd',V,V,V,V,FMatrix_old, optimize='greedy')# error!
    #FMatrix_new = np.einsum('ae,bf,cg,dh,abcd -> efgh',V,V,V,V,FMatrix_old, optimize='greedy')
...
#    path = ['einsum_path', (0, 4), (0, 3), (0, 2), (0, 1)] # most efficient sum path
#    FMatrix_new = np.einsum('ae,bf,cg,dh,efgh -> abcd',V,V,V,V,FMatrix_old, optimize=path) # error!
